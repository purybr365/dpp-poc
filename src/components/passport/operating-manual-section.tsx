  ...
  return (
    <Card>
      ...
      <CardContent className="space-y-4">
        <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
          {!!data.manualUrl && (
            <div className="bg-blue-50 p-4 rounded-lg">
              <span className="text-xs text-blue-400 block mb-1">{t("manual.productManual")}</span>
              <span className="text-sm font-medium text-blue-700">{String(data.manualUrl)}</span>
            </div>
          )}
          {!!data.warrantyDuration && (
            <div className="bg-slate-50 p-4 rounded-lg">
              <span className="text-xs text-slate-400 block mb-1">{t("manual.warranty")}</span>
        ...
        {customerSupport && (
          <div>
            <span className="text-xs text-slate-400 block mb-2">{t("manual.customerSupport")}</span>
        ...
        {!!data.safetyInfo && (
          <div className="bg-amber-50 border border-amber-200 rounded-lg p-3">
            <span className="text-xs text-amber-500 block mb-1">{t("manual.safetyInfo")}</span>
